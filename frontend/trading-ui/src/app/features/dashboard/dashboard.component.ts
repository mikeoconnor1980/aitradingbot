...
export class DashboardComponent implements OnInit {
  ...
  private _fetchAllData(): Observable<unknown> {
    ...
    return forkJoin({
      ...
      drawdownState: this._apiService.getDrawdownState(skipCtx).pipe(
        catchError(() => of(null))
      ),
      ...
      tap((results) => {
        const failedCount = [results.account, results.drawdownState, results.portfolioHeat, results.positions, results.orders]
        ...
        if (failedCount === 4) {
          ...
          if (results.drawdownState !== null) { this.drawdownState = results.drawdownState; }
