...
@Component({ selector: "app-analyst-conversation", standalone: true, imports: [CommonModule, FormsModule, MatButtonModule, MatIconModule, MatProgressSpinnerModule, AnalystEvidenceCardComponent], templateUrl: "./analyst-conversation.component.html", styleUrl: "./analyst-conversation.component.scss" })
export class AnalystConversationComponent implements AfterViewChecked {
  private readonly _router = inject(Router);
  private shouldScroll = true;
  @Input() public compact = false;
  @ViewChild("composer") public composer?: ElementRef<HTMLTextAreaElement>;
  @ViewChild("transcript") public transcript?: ElementRef<HTMLElement>;
  public readonly session = inject(AnalystSessionService);
  public readonly suggestions = ["What is happening in this chart?", "What should I watch next?"];
  public ngAfterViewChecked(): void { if (this.shouldScroll && this.transcript) { this.transcript.nativeElement.scrollTop = this.transcript.nativeElement.scrollHeight; this.shouldScroll = false; } }
  public focusComposer(): void { this.composer?.nativeElement.focus(); }
  public onPromptChange(prompt: string): void { this.session.setPrompt(prompt); }
  public onSubmit(): void { this.session.submit(); this.shouldScroll = true; }
  public onSuggestion(suggestion: string): void { this.session.submit(suggestion); this.shouldScroll = true; }
  public onKeydown(event: KeyboardEvent): void { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); this.onSubmit(); } }
  public evidence(invocations: AnalystToolInvocation[]): AnalystToolInvocation[] { return invocations.filter((invocation) => invocation.result || invocation.errorCode); }
  public contextLabel(): string { return this.session.context()?.intent === "AnalyseChart" ? "Current chart context" : "Current context"; }
  public shouldExpandEvidence(content: string): boolean { return /\b(explain|why|compare|evidence|rule)\b/i.test(content); }
  public hasReference(invocation: AnalystToolInvocation): boolean { return !!invocation.result && (typeof invocation.result["symbol"] === "string" || typeof invocation.result["strategyId"] === "string" || typeof invocation.result["backtestId"] === "string"); }
  public openReference(invocation: AnalystToolInvocation): void {
    ...
    if (symbol && invocation.toolName.includes("market")) void this._router.navigate(["/market-data"], { queryParams: { symbol } });
    else if (strategyId) void this._router.navigate(["/strategies", strategyId, "edit"]);
    else if (backtestId) void this._router.navigate(["/backtesting"], { queryParams: { viewResult: backtestId } });
  }
}

/* Duplicate conversation implementations retained below for recovery while the panel work is consolidated.
import { CommonModule } from "@angular/common";
import { AfterViewChecked, Component, ElementRef, Input, ViewChild, inject } from "@angular/core";
import { FormsModule } from "@angular/forms";
import { MatButtonModule } from "@angular/material/button";
...
export class AnalystConversationComponent implements AfterViewChecked {
  ...
  public readonly session = inject(AnalystSessionService);
  public readonly suggestions = ["What is happening in this chart?", "What should I watch next?"];
  public ngAfterViewChecked(): void { if (this._shouldScroll && this.transcript) { this.transcript.nativeElement.scrollTop = this.transcript.nativeElement.scrollHeight; this._shouldScroll = false; } }
  public focusComposer(): void { this.composer?.nativeElement.focus(); }
  public onPromptChange(prompt: string): void { this.session.setPrompt(prompt); }
  public onSubmit(): void { this.session.submit(); this._shouldScroll = true; }
  public onSuggestion(suggestion: string): void { this.session.submit(suggestion); this._shouldScroll = true; }
  public onKeydown(event: KeyboardEvent): void { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); this.onSubmit(); } }
  public evidence(invocations: AnalystToolInvocation[]): AnalystToolInvocation[] { return invocations.filter((invocation) => invocation.result || invocation.errorCode); }
  public contextLabel(): string { return this.session.context()?.intent === "AnalyseChart" ? "Current chart context" : "Current context"; }
}import { CommonModule } from "@angular/common";
...
export class AnalystConversationComponent implements AfterViewChecked {
  ...
  public get suggestions(): string[] {
    return this.session.context()?.intent === "AnalyseChart"
      ? ["What is happening in this chart?", "What should I watch next?"]
      : ["What needs my attention?", "Explain the current BTC regime", "Why have my strategies not traded?"];
  }
  ...
  public contextLabel(): string {
    return this.session.context()?.intent === "AnalyseChart" ? "Current chart context" : "Current context";
  }
  ...
  public hasReference(invocation: AnalystToolInvocation): boolean {
    return !!invocation.result && typeof invocation.result["symbol"] === "string";
  }
