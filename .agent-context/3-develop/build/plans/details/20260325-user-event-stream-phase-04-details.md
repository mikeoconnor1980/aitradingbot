...
- **Complexity**: Medium
- **Risk Factors**: Must not break existing polling-based data flow; positions and orders must continue to update even before any SignalR events arrive; must preserve existing `_pendingOrderIds` and `_pendingPositionKeys` optimistic update guards during the refactor to shared state
- **Files**:
...
**Important**: The existing dashboard uses `_pendingOrderIds` and `_pendingPositionKeys` sets to suppress poll overwrites during in-flight mutations (e.g. after placing an order, the local optimistic state is preserved until the next poll confirms). When refactoring to `AccountStateService`, these guards must be preserved. The `updatePositions()` and `updateOrders()` calls from polling should still skip items in the pending sets. Move the pending guard logic into the polling subscription callback, not into `AccountStateService` itself (keep the service generic).
