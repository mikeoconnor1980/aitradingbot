...
- **Files**:
  - `src/TradingApp.Domain/Entities/BacktestRun.cs` — Add two new properties
- **Success**:
...
Based on `src/TradingApp.Domain/Entities/BacktestRun.cs` — follows existing private setter convention.
...
- **Files**:
  - `src/TradingApp.Domain/Entities/BacktestRun.cs` — Update `CreateQueued` signature and body
- **Success**:
...
Based on `src/TradingApp.Domain/Entities/BacktestRun.cs` lines 44–83 — existing `CreateQueued` factory.
...
- **Files**:
  - `src/TradingApp.Persistence/TradingAppDbContext.cs` — Add property config + index inside `BacktestRun` entity block
- **Success**:
...
Based on `src/TradingApp.Persistence/TradingAppDbContext.cs` — inline `OnModelCreating` config convention, no separate `IEntityTypeConfiguration<T>` files.
...
```bash
dotnet ef migrations add AddStrategyLinkToBacktestRuns --startup-project ../TradingApp.Api
```
...
- **Files**:
  - `src/TradingApp.Application/Abstractions/Repositories/IBacktestRunRepository.cs` — Add new method signature
  - `src/TradingApp.Persistence/Repositories/BacktestRunRepository.cs` — Add implementation with strategy filter
- **Success**:
...
Based on `src/TradingApp.Persistence/Repositories/BacktestRunRepository.cs` — existing `GetPagedSummariesAsync` method.
...
- **Risk Factors**: None
- **Files**:
  - `src/TradingApp.Application/Backtesting/Models/BacktestRunSummary.cs` — Add 3 new properties
- **Success**:
...
```csharp
// src/TradingApp.Application/Backtesting/Models/BacktestRunSummary.cs — modification
...
Based on `src/TradingApp.Application/Backtesting/Models/BacktestRunSummary.cs` — existing `required` init property pattern. New fields are nullable (not `required`) since they are optional.
...
- **Files**:
  - `tests/TradingApp.Domain.Tests/Entities/BacktestRunTests.cs` — New test class for `BacktestRun` entity (new file)
  - `tests/TradingApp.Persistence.Tests/Repositories/BacktestRunRepositoryTests.cs` — Add tests for `GetPagedSummariesByStrategyAsync` (new file or append to existing)
- **Success**:
...
```csharp
// tests/TradingApp.Domain.Tests/Entities/BacktestRunTests.cs — new file
...
Based on `tests/TradingApp.Domain.Tests/Entities/StrategyTests.cs` (domain test pattern) and `tests/TradingApp.Persistence.Tests/Repositories/BacktestRunRepositoryTests.cs` (persistence test pattern with SQLite in-memory).
...
- **Success**:
  ...
  - `dotnet test tests/TradingApp.Domain.Tests` passes
  - `dotnet test tests/TradingApp.Persistence.Tests` passes
