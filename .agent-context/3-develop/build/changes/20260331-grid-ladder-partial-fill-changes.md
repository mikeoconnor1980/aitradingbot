...
- tests/TradePilot.Application.Tests/Trading/Services/GridControllerTests.cs: Added 12 unit tests covering deploy behavior, partial-fill lifecycle handling, fully-filled behavior, closing behavior, stop-loss priority, and open-position fallback paths.

### Modified

<!-- Phase 1: GridController Lifecycle Fix + Unit Tests -->
- src/TradePilot.Application/Trading/Services/GridController.cs: Refactored the controller to preserve partially filled ladders, evaluate TP on candle close for partial fills, and keep stop-loss priority across open-position states.
- src/TradePilot.Application/Backtesting/Models/CancellationReason.cs: Renamed `PositionOpened` to `TakeProfitTriggered` for clearer audit semantics.
- src/TradePilot.Application/Trading/Models/OrderRequest.cs: Added `CloseReason` propagation so simulated exits can record whether a close came from TP or stop loss.
- src/TradePilot.Application/Backtesting/Models/SimulatedOrder.cs: Added `CloseReason` storage for simulated open orders.
- src/TradePilot.Application/Backtesting/Models/SimulatedFill.cs: Added `CloseReason` to captured fills for downstream audit and cycle summaries.
- src/TradePilot.Application/Backtesting/Services/SimulatedExecutionEngine.cs: Propagated close reasons from simulated orders into fills.
- src/TradePilot.Application/Backtesting/Services/BacktestRunner.cs: Updated grid-cycle audit tracking to infer take-profit versus stop-loss from close reasons on fills.
- src/TradePilot.Application/Trading/Services/BacktestPositionManager.cs: Updated take-profit cancellation logging to use the renamed cancellation reason.
...
- tests/TradePilot.Api.Tests/Controllers/BacktestsControllerTests.cs: Updated API-facing backtest fixture data to use `CancellationReason.TakeProfitTriggered`.

<!-- Phase 2: Integration Tests + Knowledge Documentation -->
- tests/TradePilot.Application.Tests/Backtesting/Services/RealBacktestRunnerTests.cs: Added multi-level integration coverage for partial-fill TP, partial-fill stop-loss, fully filled limit TP, and updated the initial-market entry scenario for the corrected lifecycle.
...
- `dotnet build`: PASSED.
- `dotnet test tests/TradePilot.Application.Tests --filter "FullyQualifiedName~GridController"`: PASSED (12/12).
- `dotnet test tests/TradePilot.Application.Tests --filter "FullyQualifiedName~RealBacktestRunner"`: PASSED (3/3).
- `dotnet test`: PASSED.
- `TradePilot.Domain.Tests`: PASSED (15/15).
- `TradePilot.Application.Tests`: PASSED (76/76).
- `TradePilot.Infrastructure.Tests`: PASSED (51/51).
- `TradePilot.Persistence.Tests`: PASSED (20/20).
- `TradePilot.Api.Tests`: PASSED (145/145).
- Architecture Tests: Not run.

<!-- Phase 2: Integration Tests + Knowledge Documentation -->
- `dotnet build TradePilot.sln`: PASSED.
- `dotnet test tests/TradePilot.Application.Tests`: PASSED (79/79).
- `dotnet test tests/TradePilot.Domain.Tests`: PASSED (15/15).
- `dotnet test tests/TradePilot.Api.Tests`: PASSED (145/145).
- `dotnet test tests/TradePilot.Infrastructure.Tests`: PASSED (51/51).
- `dotnet test tests/TradePilot.Persistence.Tests`: PASSED (20/20).

## Issues

<!-- Phase 1: GridController Lifecycle Fix + Unit Tests -->
- `dotnet build` and `dotnet test` continue to report existing `NU1903` warnings for `AutoMapper` 12.0.1 in `TradePilot.Application`; these warnings pre-existed this phase and did not block compilation or tests.
...
Implemented the partial-fill grid lifecycle fix across controller logic, audit plumbing, regression tests, and agent knowledge documentation. The final regression pass succeeded across the solution (`TradePilot.Application.Tests` 79/79, `TradePilot.Domain.Tests` 15/15, `TradePilot.Infrastructure.Tests` 51/51, `TradePilot.Persistence.Tests` 20/20, `TradePilot.Api.Tests` 145/145), with only the pre-existing `AutoMapper` NU1903 warning remaining.