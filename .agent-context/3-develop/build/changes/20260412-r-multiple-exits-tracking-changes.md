...
- src/TradingApp.Persistence/Migrations/20260412181404_AddRMultipleMetrics.cs: Added nullable Expectancy, ProfitFactor, and Sqn columns to BacktestRuns.
- src/TradingApp.Persistence/Migrations/20260412181404_AddRMultipleMetrics.Designer.cs: Added the EF migration designer for the R-metrics schema change.
...
- src/TradingApp.Application/StrategyAuthoring/Models/ExitRuleType.cs: Added the RMultiple enum member so strategy configs can express take-profit targets in R.
- src/TradingApp.Application/StrategyAuthoring/Validation/BusinessRuleValidator.cs: Added R-multiple-specific take-profit validation for negative and sub-1R values while preserving the base positive-value rule.
- src/TradingApp.Application/StrategyAuthoring/Validation/CrossFieldValidator.cs: Enforced RiskBased sizing for R-multiple take-profit exits without duplicating existing stop-loss requirements.
- tests/TradingApp.Application.Tests/StrategyAuthoring/Validation/BusinessRuleValidatorTests.cs: Added coverage for negative, sub-1R, and valid R-multiple take-profit validation paths.
- tests/TradingApp.Application.Tests/StrategyAuthoring/Validation/CrossFieldValidatorTests.cs: Added coverage for RiskBased gating and stop-loss interaction with R-multiple take-profit rules.

<!-- Phase 2: R-Multiple TP Price Calculation -->
- src/TradingApp.Application/Trading/Services/TriggerOrderManager.cs: Added R-multiple take-profit price calculation and reused resolved stop-loss percent when placing or updating TP triggers.
- src/TradingApp.Application/Trading/Services/GridController.cs: Replaced duplicated inline take-profit math with a shared helper that supports R-multiple targets.
- tests/TradingApp.Application.Tests/Trading/Services/TriggerOrderManagerTests.cs: Added long and short R-multiple take-profit calculation coverage plus guard cases for missing stop-loss input.

<!-- Phase 3: Per-Trade R Tracking & MFE/MAE -->
- src/TradingApp.Application/Trading/Models/GridState.cs: Added InitialRDollars so active grid cycles can retain their captured one-R dollar risk.
- src/TradingApp.Application/Trading/Services/PositionSizeResolver.cs: Added ResolveInitialR to expose RiskBased one-R calculation without changing existing notional resolution behavior.
- src/TradingApp.Application/Trading/Services/GridController.cs: Captured InitialRDollars at grid deployment time and cleared it when grids move into closing paths.
- src/TradingApp.Application/Backtesting/Models/BacktestTrade.cs: Added nullable InitialRDollars, RMultipleResult, MFE, and MAE fields for per-trade R analytics.
- src/TradingApp.Application/Backtesting/Services/BacktestRunner.cs: Threaded InitialR through trade pairing, tracked per-candle excursion, and computed realised R metrics at close.
- tests/TradingApp.Application.Tests/Trading/Services/PositionSizeResolverTests.cs: Added RiskBased and non-RiskBased coverage for ResolveInitialR.
- tests/TradingApp.Application.Tests/Trading/Services/GridControllerTests.cs: Added coverage for InitialR capture on deploy and clearing on stop-loss close.
- tests/TradingApp.Application.Tests/Backtesting/Services/BacktestRunnerTests.cs: Added direct trade-log tests for InitialR threading, RMultipleResult calculation, MFE/MAE capture, and null R metrics for non-RiskBased trades.
- tests/TradingApp.Application.Tests/Backtesting/Services/RealBacktestRunnerTests.cs: Added an integration test covering risk-based backtests with populated R metrics.

<!-- Phase 4: Aggregate R Metrics & API -->
- src/TradingApp.Application/Backtesting/Models/BacktestResult.cs: Added aggregate R-metric fields and distribution output for risk-based backtest summaries.
- src/TradingApp.Application/Backtesting/Services/BacktestMetricsCalculator.cs: Added expectancy, profit factor, SQN, average win and loss R, win rate, and distribution calculations from R-tracked trades.
- src/TradingApp.Domain/Entities/BacktestRun.cs: Added persisted R-metric fields and updated completion methods to accept them.
- src/TradingApp.Persistence/TradingAppDbContext.cs: Configured EF persistence for the new nullable BacktestRun R-metric columns.
- src/TradingApp.Persistence/Migrations/TradingAppDbContextModelSnapshot.cs: Updated the EF snapshot to include the BacktestRun R-metric columns.
- src/TradingApp.Api/Services/BacktestProcessorService.cs: Passed aggregate R metrics into BacktestRun completion persistence.
- src/TradingApp.Application/Backtesting/Models/BacktestRunResponse.cs: Added aggregate R-metric response fields.
- src/TradingApp.Application/Backtesting/Models/BacktestTradeResponse.cs: Added trade-level R response fields.
- src/TradingApp.Application/Backtesting/BacktestRunResponseMapper.cs: Mapped persisted R metrics and derived fallback values from TradesJson for older runs.
- tests/TradingApp.Application.Tests/Backtesting/Services/BacktestMetricsCalculatorTests.cs: Added aggregate R-metric calculation coverage.
- tests/TradingApp.Persistence.Tests/Repositories/BacktestRunRepositoryTests.cs: Added persistence assertions for the new BacktestRun R fields.
- tests/TradingApp.Api.Tests/Controllers/BacktestsControllerTests.cs: Added API response coverage for aggregate and trade-level R metrics.
- tests/TradingApp.Api.Tests/Controllers/StrategiesControllerTests.cs: Updated test helpers for the BacktestRun completion signature changes.
...
- TradingApp.sln test --no-build: 7 projects completed successfully before the run stalled in TradingApp.Api.Tests.
- tests/TradingApp.Api.Tests/TradingApp.Api.Tests.csproj diagnostic run with blame-hang: remained in Testing state past 166.6 seconds.
...
- TradingApp.sln test final run: 1038/1038 passed.

<!-- Phase 3: Per-Trade R Tracking & MFE/MAE -->
- TradingApp.Application.Tests focused suite: 62/62 passed.
- TradingApp.sln build: passed.
- TradingApp.sln test: 1046/1046 passed.

<!-- Phase 4: Aggregate R Metrics & API -->
- TradingApp.sln build: passed.
- TradingApp.sln test --no-build: 1050/1050 passed.
- dotnet ef migrations add AddRMultipleMetrics --startup-project ../TradingApp.Api --no-build: passed.
...
<!-- Phase 1: Domain Models & Validation -->
- A running TradingApp.Api process locked build outputs during the first verification attempt; process 69260 was stopped and the build succeeded on rerun.
- Full-solution test verification could not be completed cleanly because TradingApp.Api.Tests stalled without surfacing a failing test.
...
- Investigate the stalled tests/TradingApp.Api.Tests/TradingApp.Api.Tests.csproj run separately before treating whole-solution verification as clean.

<!-- Phase 2: R-Multiple TP Price Calculation -->
- Review the shared take-profit helper in src/TradingApp.Application/Trading/Services/GridController.cs to confirm the current long-side controller assumption is still intentional.
- Review the timing sensitivity in tests/TradingApp.Api.Tests/Services/CandleIngestionServiceTests.cs because it can fail under full-suite load even when reruns pass.

<!-- Phase 3: Per-Trade R Tracking & MFE/MAE -->
- Review the partial-close path in src/TradingApp.Application/Backtesting/Services/BacktestRunner.cs because it is the area most sensitive to future partial-fill semantics.
- Review the new risk-based integration coverage in tests/TradingApp.Application.Tests/Backtesting/Services/RealBacktestRunnerTests.cs if exact MFE and MAE thresholds need to be tightened later.

<!-- Phase 4: Aggregate R Metrics & API -->
- Review src/TradingApp.Application/Backtesting/BacktestRunResponseMapper.cs closely because it now serves both newly persisted runs and historical runs via a fallback derivation path.
- Review src/TradingApp.Persistence/Migrations/TradingAppDbContextModelSnapshot.cs for migration ordering and snapshot consistency, especially if other persistence work lands nearby.
- Review tests/TradingApp.Api.Tests/Controllers/BacktestsControllerTests.cs if exact SQN assertions need to be tightened beyond non-null coverage.
