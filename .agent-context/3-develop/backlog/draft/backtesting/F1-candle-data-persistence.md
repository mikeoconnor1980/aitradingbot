...
- [ ] EF Core migrations are used to create and evolve the database schema
- [ ] The SQLite database file path is configurable via `appsettings.json` under `ConnectionStrings:DefaultConnection` (default: `Data Source=Data/tradingapp.db`)
...
- [ ] `BulkInsertAsync` uses upsert semantics — duplicates are skipped based on the composite unique index (no errors on re-insert)
...
- [ ] Should the `Data/` directory be relative to the working directory or configurable as an absolute path?
- [ ] Should the initial migration be applied automatically on startup via `context.Database.MigrateAsync()` or require manual `dotnet ef database update`?
