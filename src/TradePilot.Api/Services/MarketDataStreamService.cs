...
public sealed class MarketDataStreamService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _wsClient.OnConnectionStateChanged(async state =>
        {
            ...
            try
            {
                await _publisher.BroadcastConnectionStatusAsync(status, CancellationToken.None);
            }
            catch (Exception ex)
            {
    ...
    private async Task ResyncStateFromRestAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            await _publisher.BroadcastOrdersResyncAsync(
                orders,
                cancellationToken);

            await _publisher.BroadcastPositionsResyncAsync(
                positions,
                cancellationToken);
