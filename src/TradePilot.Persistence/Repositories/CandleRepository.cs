...
{
    private const int BatchSize = 200; // 10 params/row × 200 = 2000 (SQL Server limit: 2100)
            ...
            if (isInMemory)
            {
                foreach (var candle in batch)
                {
                    var exists = await _context.Candles.AnyAsync(
                        c => c.Source == candle.Source && c.Symbol == candle.Symbol
                             && c.Interval == candle.Interval && c.Timestamp == candle.Timestamp,
                        cancellationToken);
                    if (!exists)
